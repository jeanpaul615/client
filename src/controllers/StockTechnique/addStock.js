...
export const addStock = async (stockData) => {
  try {
    const response = await fetch('http://3.136.228.49:8001/add-stocksistema', {
      method: 'POST',
      headers: {
...
export const getTechnicians = async () => {
  try {
    const response = await fetch('http://3.136.228.49:8001/tecnico/tecnicos');
    if (!response.ok) {
      throw new Error(`Error en la solicitud: ${response.statusText}`);
    }
...
export const getMaterials = async () => {
  try {
    const response = await fetch('http://3.136.228.49:8001/stock/get-stocksistema');
    if (!response.ok) {
      throw new Error(`Error en la solicitud: ${response.statusText}`);
    }
    const data = await response.json(); // Corregido para obtener datos JSON
...
export const getStockByMaterial = async (Nombre_material) => {
  try {
    const response = await fetch('http://3.136.228.49:8001/stock/byname', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
